...
class TestNoteListCreateAPIView(APITestCase):
    ...
    def setUpTestData(cls): # тест данные один раз на все тесты
        User.objects.create(username='test@test.ru')   # создаем пользователя по умолчанию  у него author_id=1
    ...
    def test_empty_list_objects(self):
        ...
        # ожидаем от сервера ответ "status.HTTP_200_OK"
    ...
    def test_list_objects(self):
        ...
        Note.objects.create(title='Заголовок из тестов 1', author_id=1)   # создание 1 запись
        # аналогично по пользователю
        test_user = User.objects.get(username="test@test.ru")
        Note.objects.create(title='Заголовок из тестов 1', author=test_user)
